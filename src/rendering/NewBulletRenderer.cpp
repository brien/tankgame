...
#include "NewBulletRenderer.h"
#include "RenderData.h"
#include "../App.h"
#include "../Logger.h"

NewBulletRenderer::NewBulletRenderer() : 
    BaseRenderer(),
    blendEnabled(false),
...
bool NewBulletRenderer::Initialize() {
    ...
    Logger::Get().Write("NewBulletRenderer initialized successfully\n");
    return true;
}

void NewBulletRenderer::Cleanup() {
    BaseRenderer::Cleanup();
    Logger::Get().Write("NewBulletRenderer cleaned up\n");
}

void NewBulletRenderer::RenderBullets(const std::vector<BulletRenderData>& bullets) {
    if (!IsReady()) {
        Logger::Get().Write("ERROR: NewBulletRenderer not initialized\n");
        return;
    }
...
void NewBulletRenderer::RenderBlueBullet(const BulletRenderData& bullet) {
...
void NewBulletRenderer::RenderStandardBullet(const BulletRenderData& bullet) {
    // Standard bullets render as a single geometric piece
    RenderBulletGeometry(bullet, -0.05f, 0.0f, 0.0f, 0.2f);
}

void NewBulletRenderer::RenderBulletGeometry(const BulletRenderData& bullet, float yOffset, float zOffset, float rotationX, float scaleZ) {
    glPushMatrix();
...
void NewBulletRenderer::RestoreBlendMode() {
    if (blendEnabled) {
        glDisable(GL_BLEND);
        glDepthMask(GL_TRUE);
